...
export class App extends PureComponent {
  ...
  async _removeTab(tab) {
    ...
    const { navigationHistory, closedTabs } = this;
  ...
  componentDidUpdate(prevProps, prevState) {
    const { activeTab, tabs, tabLoadingState, tabState, layout, endpoints } =
      this.state;
  ...
  workspaceChanged = (debounce = true) => {
    ...
    const { layout, tabs, activeTab, endpoints } = this.state;
  ...
  /**
   * Simulates current tab
   */
  async simulateTab(tab, options) {
    const {
      file: { path },
    } = tab;
    const { contents } = options;

    const simuAPI = this.getGlobal("simuAPI");

    const result = await simuAPI.simulate({
   ...
   * Simulates current tab
   */
  async abortSimulations(options) {
    const simuAPI = this.getGlobal("simuAPI");

    return simuAPI.abortSimulations();
  }
  ...
  revealTabInFileExplorer = (matcher) => {
    ...
    return Promise.reject(
      new Error("cannot open file explorer: tab not found or no path")
    );
  };
  ...
  triggerAction = failSafe((action, options) => {
    ...
    if (action === "create-simu-bpmn-diagram") {
      return this.createDiagram("simu-bpmn");
    }
    ...
    if (action === "reveal-tab") {
      return this.revealTabInFileExplorer(
        (t) => options && t.id === options.tabId
      );
    }
    ...
    if (action === "simulate") {
      return this.simulateTab(activeTab, options);
    }

    if (action === "abort-simulate") {
      return this.abortSimulations(options);
    }
  ...
  render() {
    const { tabs, activeTab, layout, logEntries, dirtyTabs, unsavedTabs } =
      this.state;
    ...
    const groupedItems = map(groupBy(items, "group"), (group, key) => {
      ...
      if (key === "Camunda Platform 8") {
        item.labelSuffix = (
          <NewBadge
            inline
            style={{ marginLeft: "20px", transform: "translate(0px, -1px)" }}
          />
        );
      }
