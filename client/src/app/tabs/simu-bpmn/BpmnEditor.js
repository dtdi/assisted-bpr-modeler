...
export class BpmnEditor extends CachedComponent {
  ...
  listen(fn) {
    ...
    modeler[fn]("abpr.simulate", this.handleSimulation);

    modeler[fn]("abpr.simulate.abort", this.handleSimulationAbort);
  ...
  handleLinting = () => {
    const { engineProfile, modeler } = this.getCached();
  ...
  handleSimulationAbort = async () => {
    const { onAction } = this.props;
    return await onAction("abort-simulate", {});
  };
  ...
  render() {
    ...
    return (
      <div className={css.BpmnEditor}>
        ...
        {engineProfile && (
          <Fragment>
            <Panel layout={layout} onUpdateMenu={onUpdateMenu}>
              <LintingTab
                layout={layout}
                linting={linting}
                onAction={onAction}
