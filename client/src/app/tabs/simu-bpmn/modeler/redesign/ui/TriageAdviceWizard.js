...
export default class TriageAdviceWizard extends ElementComponent {
  ...
  niceName = (bO) => {
    ...
    const incoming = elem.incoming[0] && elem.incoming[0].bsim;
    if (incoming) {
                ...
                <StackItem grow>
                  <Stack horizontal verticalAlign={"end"}>
                    <StackItem grow={3}>
                      <Slider
                        value={
                          incoming.bsim.get("branchingProbability") &&
                          incoming.bsim.get("branchingProbability").value
                        }
                        max={1}
                        label={`${this._getName(
                          null,
