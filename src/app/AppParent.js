...
import { assign, forEach, isString } from "min-dash";
...
export default class AppParent extends PureComponent {
  ...
  triggerAction = (event, action, options) => {
    ...
    const exec = async () => {
      ...
      const { backend } = this.props.globals;
  ...
  restoreWorkspace = async () => {
    ...
    const { activeFile, files, layout } = restored;
  ...
  handleError = async (error, source) => {
    ...
    if (this.hasPlugins()) {
      ...
      this.logToClient(
        getClientEntry(
          "info",
          "Disable plug-ins (restarts the app)",
          this.togglePlugins
        )
      );
    }
  ...
  getErrorMessage(categoryOrTab) {
    const prefix = categoryOrTab
      ? isString(categoryOrTab)
        ? categoryOrTab
        : "tab"
      : "app";
  ...
  handleStarted = async () => {
    ...
    const { files, activeFile } = prereadyState;
  ...
  componentDidMount() {
    ...
    */
    ...
    if (Flags.get(DISABLE_PLUGINS) && Flags.get(RELAUNCH)) {
      this.logToClient(
        getClientEntry("info", "Plugins are temporarily disabled.")
      );

      this.logToClient(
        getClientEntry(
          "info",
          "Enable plug-ins (restarts the app)",
          this.togglePlugins
        )
      );
    }
  ...
  render() {
    const { tabsProvider, globals } = this.props;
...
function mergeFiles(oldFiles, newFiles) {
  const actualNewFiles = newFiles.filter(
    (newFile) => !oldFiles.some((oldFile) => oldFile.path === newFile.path)
  );

  return [...oldFiles, ...actualNewFiles];
}
...
async function getEntryMessage(errorLike, tab) {
  const { message: originalMessage, stack } = errorLike;
