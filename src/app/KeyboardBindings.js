...
import { findIndex, forEach, isArray, omit } from "min-dash";
...
export default class KeyboardBindings {
   ...
   */
  constructor(options = {}) {
    const { menu, onAction, isMac } = options;

    this.onAction = onAction;

    (this.isMac = isMac), (this.copy = null);
    this.cut = null;
    this.paste = null;
    this.selectAll = null;
  ...
  _keyDownHandler = (event) => {
    ...
    if (
      commandOrCtrl &&
      isPaste(event) &&
      isEnabled(this.paste) &&
      !hasRole(this.paste, "paste")
    ) {
    ...
    // remove selection
    if (
      isSecondaryRemoveSelection(event, isMac) &&
  ...
  updateRemoveSelectionEntry(menu) {
    ...
    // (2) register it
    this.removeSelection = find(menu, ({ accelerator }) =>
      isAccelerator(accelerator, primaryAccelerator)
    );
  ...
  updateCustomEntries(menu) {
    this.customEntries = getCustomEntries(menu).reduce(
      (customEntries, entry) => {
        const { custom } = entry;
...
function isUndo(event) {
  return (
    isKey(["z", "Z"], event) && isCommandOrControl(event) && !isShift(event)
  );
}
...
export function findAll(menu = [], matcher) {
  return menu.reduce((found, entry) => {
    if (isArray(entry)) {
      return [...found, ...findAll(entry, matcher)];
    }
    ...
    if (entry.submenu) {
      found = [...found, ...findAll(entry.submenu, matcher)];
    }
...
function findCopy(menu) {
  return find(menu, ({ accelerator }) =>
    isAccelerator(accelerator, "CommandOrControl+C")
  );
}

function findCut(menu) {
  return find(menu, ({ accelerator }) =>
    isAccelerator(accelerator, "CommandOrControl+X")
  );
}

function findPaste(menu) {
  return find(menu, ({ accelerator }) =>
    isAccelerator(accelerator, "CommandOrControl+V")
  );
}

function findUndo(menu) {
  return find(menu, ({ accelerator }) =>
    isAccelerator(accelerator, "CommandOrControl+Z")
  );
}

function findRedo(menu) {
  return find(menu, ({ accelerator }) =>
    isAccelerator(accelerator, "CommandOrControl+Y")
  );
}

function findSelectAll(menu) {
  return find(menu, ({ accelerator }) =>
    isAccelerator(accelerator, "CommandOrControl+A")
  );
}
