...
import { TabLinks, TabContainer } from "../primitives";
...
export class MultiSheetTab extends CachedComponent {
  ...
  async showImportErrorDialog(error) {
    const { onAction, tab } = this.props;

    const { name, type } = tab;

    const { button } = await onAction(
      "show-dialog",
      getErrorDialog({
        error,
        name,
  ...
  displayImportWarningsNotification(warnings) {
    this.props.onAction("display-notification", {
      type: "warning",
      title: `Imported with ${warnings.length} warning${
        warnings.length !== 1 ? "s" : ""
      }`,
  ...
  render() {
    ...
    let { id, xml, layout, tab } = this.props;
...
function getErrorDialog({ error, name, type }) {
  return {
    type: "error",
    title: "Import Error",
    message: "Ooops!",
