...
export default class Distribution extends ElementComponent {
  ...
  inflateDistr(distribution) {
    ...
    const distributionDescriptor = moddle.getElementDescriptor(
      distributionType
    );
    ...
    distributionDescriptor.properties
      ...
      .forEach((prop) => {
        if (prop.name === "entry") {
          ...
          properties.push({
            name: prop.name,
            display: _type?.meta?.displayName || this.name(prop.name),
            description: _type?.meta.description,
            value: distribution.get(prop.name)?.value,
          });
        }
      });
  ...
  triggerChange(newState) {
    ...
    if (distribution.$type === "bsim:arbitraryFiniteProbabilityDistribution") {
      ...
      newState.distributionDescriptor.properties.forEach((prop) => {
        const value = newState.properties.find((e) => e.name === prop.name)
          .value;
        if (!value) {
          isComplete = false;
        }
    ...
    const distributions = moddle
      ...
      .map((e) => {
        return {
          ...e,
          key: e.name,
          text: e.meta?.displayName || this.name(e.name),
          data: { icon: e.name },
        };
      });
    ...
    return (
      <Stack tokens={{ childrenGap: 5 }} verticalAlign="end">
        ...
        <Dropdown
          ...
          selectedKey={distributionDescriptor?.ns.localName}
          onChange={this.onDistrChange}
        />
        <Text variant="xSmall">
          {this._getDescr(null, distributionDescriptor?.name)}
          <WikiHelp
