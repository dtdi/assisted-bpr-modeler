        ...
        <DefaultButton
          ...
          onClick={() => {
            ...
            onChange(task, "bsim:resources", {
              resource: [...(task.resources?.resource || []), ...newRes],
            });
          }}
        />
      </div>
  ...
  render() {
    ...
    return (
      <Stack tokens={{ childrenGap: 15, padding: 15 }}>
        ...
        >
          <TimeUnit
            value={task.duration?.timeUnit}
            onChange={(ev, val) => {
              onChange(task, "bsim:duration", { "bsim:timeUnit": val.key });
            }}
          />
          <Distribution
            distribution={task.duration?.distribution}
            onChange={(distr) => {
        ...
        >
          <TimeUnit
            value={task.setUpDuration?.timeUnit}
          ...
          />
          <Distribution
            distribution={task.setUpDuration?.distribution}
            onChange={(distr) => {
              if (!distr) {
