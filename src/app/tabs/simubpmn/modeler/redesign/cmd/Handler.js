...
Handler.prototype._evaluate = async function (
  ...
  try {
    const response = await fetch("https://abpr.dtdi.de/", {
    ...
    const bsimApiObj = await moddle.fromXML(responseXml);

    const { processes, resources } = bsimApiObj?.rootElement;

    const processSimu = processes?.process[0];
